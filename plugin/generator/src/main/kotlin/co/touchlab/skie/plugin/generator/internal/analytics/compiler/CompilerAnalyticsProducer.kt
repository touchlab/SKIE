...
class CompilerAnalyticsProducer(
    ...
    private fun getFullCompilerAnalytics(): CompilerAnalytics =
        CompilerAnalytics(
            ...
            mavenLibraries = config.getMavenLibraries(),
            cacheSupport = config.getCacheSupport(),
        )
}
...
private fun CompilerConfiguration.getCommonConfig(): CompilerAnalytics.CommonConfig =
    CompilerAnalytics.CommonConfig(
        ...
        useLightTree = get(CommonConfigurationKeys.USE_LIGHT_TREE),
...
private fun CompilerConfiguration.getBinaryConfig(): CompilerAnalytics.BinaryConfig =
    CompilerAnalytics.BinaryConfig(
        ...
        objcExportDisableSwiftMemberNameMangling = get(BinaryOptions.objcExportDisableSwiftMemberNameMangling),
        ...
        mimallocUseCompaction = get(BinaryOptions.mimallocUseCompaction),
        preview19LLVMPipeline = get(BinaryOptions.preview19LLVMPipeline),
    )

private fun KonanConfig.getMavenLibraries(): List<CompilerAnalytics.Library> {
    val exportedArtifactsPaths = resolve.exportedLibraries.map { it.libraryFile.absolutePath }.toSet()

    return userVisibleIrModulesSupport
        .reflectedBy<UserVisibleIrModulesSupportReflector>()
        .externalDependencyModules
        .map { it.toLibrary(exportedArtifactsPaths) }
}

private fun KonanConfig.getCacheSupport(): CompilerAnalytics.CacheSupport =
    CompilerAnalytics.CacheSupport(
        hasStaticCaches = cacheSupport.cachedLibraries.hasStaticCaches,
        hasDynamicCaches = cacheSupport.cachedLibraries.hasDynamicCaches,
    )
