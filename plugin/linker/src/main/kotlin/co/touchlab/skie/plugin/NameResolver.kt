...
import co.touchlab.skie.plugin.TransformAccumulator.TypeTransformTarget
...
internal class NameResolver(
    ...
    val typeNames: Map<TypeTransformTarget, MutableSwiftTypeName> = mutableTypeNames
    val functionNames: Map<FunctionDescriptor, MutableSwiftFunctionName> = mutableFunctionNames

    fun resolveName(target: TypeTransformTarget): MutableSwiftTypeName = mutableTypeNames.getOrPut(target) {
        when (target) {
            is TypeTransformTarget.Class -> resolveClassName(target)
            is TypeTransformTarget.File -> resolveFileName(target)
        }
    }
    ...
    fun resolveName(functionDescriptor: FunctionDescriptor): MutableSwiftFunctionName = mutableFunctionNames.getOrPut(functionDescriptor) {
        ...
        val (functionName, parametersString) = "(.+?)\\((.*?)\\)".toRegex().matchEntire(swiftName)?.destructured
        ...
        DefaultMutableSwiftFunctionName(
            ...
            parameterNames = parameters.map { DefaultMutableSwiftParameterName(it) },
            receiverName = resolveName(namer.getContainingTarget(functionDescriptor)),
        )
    }

    private fun resolveFileName(target: TypeTransformTarget.File): MutableSwiftTypeName = DefaultMutableSwiftTypeName(
        originalParent = null,
        originalSimpleName = namer.getFileClassName(target.file).swiftName,
    )

    private fun resolveClassName(target: TypeTransformTarget.Class): MutableSwiftTypeName {
