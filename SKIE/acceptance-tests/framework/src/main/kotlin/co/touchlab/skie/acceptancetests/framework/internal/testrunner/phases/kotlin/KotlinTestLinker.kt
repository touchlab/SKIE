...
import co.touchlab.skie.compilerinject.compilerplugin.SkieComponentRegistrar
...
class KotlinTestLinker(
    ...
    fun link(
        ...
        skieConfigurationData?.let { configureSwiftKt(skieConfigurationData) }
        ...
        return try {
            ...
            interpretResult(result, outputFile, outputStream)
        } catch (t: Throwable) {
            IntermediateResult.Error(
                TestResult.KotlinLinkingError(
    ...
    private fun configureSwiftKt(skieConfigurationData: CompilerSkieConfigurationData) {
        val buildDirectory = tempFileSystem.createDirectory("skie")
        val skieDirectories = SkieDirectories(buildDirectory.toFile())

        skieDirectories.initializeForTests(skieConfigurationData)

        PluginRegistrar.configure.set {
            put(SkieConfigurationKeys.SkieDirectories, skieDirectories)
        }
    ...
    private fun SkieDirectories.initializeForTests(skieConfigurationData: CompilerSkieConfigurationData) {
        buildDirectory.createDirectories()
        buildDirectory.resetTemporaryDirectories()

        val mergedConfiguration = skieConfigurationData + getDumpSwiftApiConfiguration()
    ...
    private fun getDumpSwiftApiConfiguration(): CompilerSkieConfigurationData =
        if ("debugDumpSwiftApi" in TestProperties) {
            CompilerSkieConfigurationData(
                enabledConfigurationFlags = setOf(
                    SkieConfigurationFlag.Debug_DumpSwiftApiBeforeApiNotes,
                    SkieConfigurationFlag.Debug_DumpSwiftApiAfterApiNotes,
                ),
            )
        } else {
            CompilerSkieConfigurationData()
