...
internal class TestNodeRunner(
    ...
    suspend fun runTests(testNode: TestNode): EvaluatedTestNode {
        ...
        val testResultReporter = TestResultReporter(tests)

        val testsWithResults = runTests(tests, testResultReporter)
    ...
    private suspend fun runTests(tests: List<TestNode.Test>, testResultReporter: TestResultReporter): Map<TestNode.Test, TestResultWithLogs> =
        withContext(testDispatcher()) {
            tests
                .map { async { it to testRunner.runTest(it, testResultReporter) } }
                .awaitAll()
                .toMap()
        }
