...
class SwiftTestCompiler(
    ...
    fun compile(kotlinFramework: Path, swiftFiles: List<Path>): IntermediateResult<Path> {
        val output = tempFileSystem.createFile("swift-binary")

        val command = createCompileSwiftCommand(kotlinFramework, swiftFiles, output)
    ...
    private fun createCompileSwiftCommand(
        ...
        swiftFiles.map { it.absolutePathString() } +
        listOf(
            "-F",
