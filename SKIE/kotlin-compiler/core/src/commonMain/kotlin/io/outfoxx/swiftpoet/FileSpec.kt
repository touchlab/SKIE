 ...
 * - Members
 */
class FileSpec private constructor(builder: Builder) : Taggable(builder.tags.toImmutableMap()) {
    ...
    private fun emit(codeWriter: CodeWriter) {
        ...
        if (imports.isNotEmpty()) {
            val flattenedImports = imports.groupBy { it.name }.mapValues { (key, importGroup) ->
                ...
                val attributesToUse = if (needsFullImport) {
                    allAttributes.filterNot {
                        it == AttributeSpec.IMPLEMENTATION_ONLY
                    }
                } else {
                    allAttributes
                }
    ...
    class Builder internal constructor(val moduleName: String, val name: String) :
    ...
    companion object {
        ...
        @JvmStatic
        fun get(moduleName: String, typeSpec: AnyTypeSpec): FileSpec = builder(moduleName, typeSpec.name).addType(typeSpec).build()
