...
internal inline fun <reified T : Enum<T>> Collection<T>.toEnumSet(): Set<T> = enumValues<T>().filterTo(mutableSetOf(), this::contains)
...
internal fun escapeKeywords(canonicalName: String) = canonicalName.split('.').joinToString(".") { escapeIfKeyword(it) }

internal fun escapeIfKeyword(value: String) = if (value.isKeyword) "`$value`" else value

internal fun escapeIfNotJavaIdentifier(value: String) = if (value.isNotEmpty() &&
    !Character.isJavaIdentifierStart(value.first()) ||
    value.drop(1).any { !Character.isJavaIdentifierPart(it) }
) {
    "`$value`"
} else {
    value
