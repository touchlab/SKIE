...
class ErasedPhaseInterceptorChain<Context, Input, Output>(interceptors: List<PhaseInterceptor<Context, Input, Output>>) :
    ...
    override fun invoke(context: Context, input: Input, original: OriginalPhaseBody<Context, Input, Output>): Output =
        chainedInterceptors(context, input, original)
}
...
): ErasedPhaseInterceptor<Context, Input, Output> = { outerContext, outerInput, original ->
