...
SKIE is a compiler plugin that complements Kotlin/Native by generating Swift wrappers for the Objective-C headers generated by the Kotlin compiler. Its goal is to improve interoperability between Kotlin and Swift by recreating features supported by both languages, but lost in the translation from Kotlin to Objective-C to Swift.
...
- It has less than ideal syntax that leads to "callback hell" (which is what suspend functions were meant to solve in the first place).
...
The callback syntax was already solved when Swift 5.5 added `async`/`await` with interop for legacy ObjC functions.
...
// The `foo` function is exported as `IKt.foo(_:)`
// (the first parameter is the extension receiver).
fun I.foo()
```
...
The Kotlin compiler does not have any specific support for Flows - they are treated as regular interfaces.
...
For now, we think it's a suitable balance between the number of supported cases (since not many functions exceed that number) and the overhead introduced in the form of compilation time and binary size.
...
the number of default arguments may change in the future as we test the plugin on larger projects.*
