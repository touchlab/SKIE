...
abstract class GeneratePrimarySmokeTestsCIActionTask : DefaultTask() {
    ...
    @TaskAction
    fun execute() {
        val numberOfLibraryTests = getNumberOfLibraryTests()

        generatePushTriggerWorkflow(numberOfLibraryTests)
        generateManualTriggerWorkflow(numberOfLibraryTests)
    }

    private fun getNumberOfLibraryTests(): Int {
    ...
    private fun generatePushTriggerWorkflow(numberOfLibraryTests: Int) {
        ...
        val workflow = getSmokeTestsPushTriggerWorkflow(numberOfLibraryTests)

        outputPath.writeText(workflow)
    }

    private fun generateManualTriggerWorkflow(numberOfLibraryTests: Int) {
        val outputPath = this@GeneratePrimarySmokeTestsCIActionTask.manualTriggerOutputPath.get().asFile.toPath()

        outputPath.createParentDirectories()

        val workflow = getSmokeTestsManualTriggerWorkflow(numberOfLibraryTests)

        outputPath.writeText(workflow)
    }

    private fun getSmokeTestsPushTriggerWorkflow(numberOfLibraryTests: Int): String = $$"""
        name: Smoke Tests
    ...
    """.trimIndent() + getSmokeTestsBaseWorkflow(numberOfLibraryTests, "")

    private fun getSmokeTestsManualTriggerWorkflow(numberOfLibraryTests: Int): String = """
    ...
    private fun getExternalLibrariesJob(
        ...
        return $$"""
              ...
              external-libraries-tests-$$range:
                name: External Libraries Tests $$range
                needs: [gradle-tests]
                runs-on: self-hosted
                steps:
                  - name: Checkout Repo
