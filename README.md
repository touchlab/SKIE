...
Skie is a compiler plugin that generates Swift wrappers of the Objective-C headers generated by the Kotlin compiler. Its goal is to improve interoperability between Kotlin and Swift.
...
This feature allows you to exhaustively switch on sealed Kotlin hierarchies from Swift. For example, consider the following Kotlin code:
...
Default arguments (or parameters) are a feature in both [Kotlin](https://kotlinlang.org/docs/functions.html#default-arguments) and [Swift](https://docs.swift.org/swift-book/LanguageGuide/Functions.html#ID169) that allows the function caller to omit function arguments when the missing arguments are provided by the called function. Unfortunately, Objective-C does not support default arguments in any way. Therefore, Swift must always specify all arguments when calling Kotlin functions.
...
Specifically, it does not properly handle inheritance, generics, and generated overloads of multiple functions with default arguments. If you run into this issue, you might have to disable the code generation for one of the functions (see [Configuration section](#configuration)).

Alternatively, you can rename one conflicting functions (or their parameters).

Since it is not possible to generate exponential numbers of functions, the number of default arguments supported is limited to 5, so that at most 31 additional functions will be generated per function with default arguments. If a function exceeds has more than 5 default arguments, SKIE will not generate any extra functions.

The number 5 was chosen as the limit after internal experimentation, however that number might change. For now, we think it's a suitable balance between the number of supported cases (since very few functions exceed that number) and the introduced overhead in compilation time and binary size.

The limit can be explicitly configured using the `DefaultArgumentInterop.MaximumDefaultArgumentCount` key/annotation (see Configuration, and specifically Annotations in the next section).
...
`group` and `from` can be freely mixed together and repeated multiple times. The file format is identical to `build/co.touchlab.skie/config.json` which contains a JSON encoding all applied configuration.
